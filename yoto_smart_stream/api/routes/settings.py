...
def get_effective_setting_value(key: str, db: Session) -> str:
    """
    Get the effective setting value (for use in application logic).

    This function returns just the effective value without override metadata.
    It's meant to be used by application code that needs the current setting value.
    ...
    Returns:
        str: Effective setting value (env var if set, otherwise database value)
    """
    effective_value, _, _ = get_setting_value(key, db)
