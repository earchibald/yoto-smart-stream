...
- ALWAYS use the option to disable the interactive pager when using certain CLI tools (e.g., `--no-pager` for `git`, `--no-cli-pager` for `aws`, etc.) to ensure output is not blocked.
